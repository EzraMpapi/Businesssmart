...
// ─── Additional utility exports needed by modules ─────────────────────────────
export const TODAY_STR = new Date().toISOString().slice(0, 10);
export const genRef = (method) => {
  const d = new Date(); const pad = n => String(n).padStart(2,'0');
  const prefix = {Cash:'CASH',Card:'CARD','M-Pesa':'MPE','Airtel Money':'AIR','Bank Transfer':'BANK','Cheque':'CHQ'}[method]||'REF';
  return `${prefix}-${pad(d.getDate())}${pad(d.getMonth()+1)}-${Math.floor(1000+Math.random()*9000)}`;
};
export const docId = (prefix = 'DOC') => `${prefix}-${Math.floor(1000+Math.random()*9000)}`;
export const inputClass = "w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-[12.5px] text-[#111827] outline-none focus:border-[#16A34A] focus:ring-1 focus:ring-[#16A34A]/20 transition-all placeholder-slate-400";
export const sortRows = (rows, sort) => { if(!sort?.field) return rows; return [...rows].sort((a,b)=>{ const v1=a[sort.field]??'', v2=b[sort.field]??''; return sort.direction==='asc'?(v1>v2?1:v1<v2?-1:0):(v1<v2?1:v1>v2?-1:0); }); };
export const toggleSort = (sort, setSort, field) => setSort(s => ({ field, direction: s.field===field && s.direction==='asc' ? 'desc' : 'asc' }));
